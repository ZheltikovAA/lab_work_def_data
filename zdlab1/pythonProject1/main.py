    ...
    while True:
        ...
        match(event):
            case 'Сменить пароль':
                ...
                elif newValuePass and values["OldPass"] == libForUser.getPassword(login)[0][0]:
                    result = libForUser.changePassword(values["RepeatNewPass"], login)
                    match (result):
                        case 200:
                            window3.close()
        ...
        match(event):
            ...
            case 'block':
                if window2['OUTPUT'].get() == 'admin':
                    sg.popup('Ты чё дурак сам себя блокировать????')
...
def mainWindowCall():
    ...
    # Create the Window
    window = sg.Window('Лабораторная работа номер 1', layout)
    ...
    while True:
        ...
        if event == 'Войти':
            ...
            match libForUser.compareInput(values['Login'], values['Password']):
                ...
                case 4: 
                    COUNT = 0 
                    # TODO Доделать в всплывающем окне ввод пароля и занесение его в бд
